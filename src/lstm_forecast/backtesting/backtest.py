...
from src.backtesting.portfolio import FakePortfolio
from src.backtesting.plot import plot_predictions_with_orders
    ...
    for i in range(len(historical_data)):
        ...
        if i < len(predictions) - 1:
            ...
            if portfolio.take_profit and portfolio.holdings.get(symbol, 0) > 0:
                avg_buy_price = sum(
                    t['price'] * t['quantity'] for t in portfolio.transaction_history if t['symbol'] == symbol and t['type'] == 'buy'
                ) / sum(t['quantity'] for t in portfolio.transaction_history if t['symbol'] == symbol and t['type'] == 'buy')
                if historical_data['Close'].values[i] >= avg_buy_price * (1 + portfolio.take_profit):
            ...
            if portfolio.stop_loss and portfolio.holdings.get(symbol, 0) > 0:
                ...
                if historical_data['Close'].values[i] <= avg_buy_price * (1 - portfolio.stop_loss):
                    portfolio.sell(symbol, portfolio.holdings.get(symbol, 0), historical_data['Close'].values[i],

...
def mock_config():
    ...
    config.all_features = ["Open", "High", "Low", "Close", "Volume", "SMA_5", "SMA_10"]
    config.look_back = 30
    config.look_forward = 1
    config.disabled_features = []
    config.data_settings = {"selected_features": ["Open", "High", "Low", "Close", "Volume"]}
...
def test_get_data(mock_calculate_indicators, mock_yf_download, mock_config, mock_historical_data, mock_features):
    mock_yf_download.return_value = mock_historical_data
    mock_calculate_indicators.return_value = (mock_historical_data, mock_features)
...
def test_create_dataset(mock_config):
    ...
    X, y = create_dataset(features, targets, look_back, look_forward, mock_config.targets)

    assert isinstance(X, np.ndarray)
    assert isinstance(y, np.ndarray)
    assert X.shape == (100 - look_back - look_forward + 1, look_back, len(selected_features))
    assert y.shape == (100 - look_back - look_forward + 1, len(mock_config.targets))
...
def test_load_and_preprocess_data(mock_train_test_split, mock_preprocess_data, mock_get_data, mock_config, mock_historical_data, mock_features):
